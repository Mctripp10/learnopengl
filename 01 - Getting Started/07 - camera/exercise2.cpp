...
glm::mat4 lookAt2(glm::vec3 position, glm::vec3 target, glm::vec3 up);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // pass projection matrix to shader (must pass in loop now for fov to update projection matrix frame by frame)
        glm::mat4 projection = glm::perspective(glm::radians(fov), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        ourShader.setMat4("projection", projection);

        // camera/view transformation
        glm::mat4 view = lookAt2(cameraPos, cameraPos + cameraFront, cameraUp);
...
glm::mat4 lookAt2(glm::vec3 position, glm::vec3 target, glm::vec3 up)
{
    ...
    glm::vec3 down = glm::normalize(glm::cross(direction, right));

    glm::mat4 lookAtMatrix = glm::mat4(1.0f);

    float rotArr[16] = { right.x, right.y, right.z, 0, up.x, up.y, up.z, 0, down.x, down.y, down.z, 0, 0, 0, 0, 1 };
    ...
    return lookAtMatrix;
    // lookAtMatrix = glm::rotate(lookAtMatrix, glm::radians(45.0f), glm::vec3())
    // // model = glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
}